...
def test_litellm_connection() -> tuple[bool, str]:
    ...
    try:
        ...
        return True, f"연결 성공 (모델: llama.cpp)"
    except ImportError:
...
def main():
    ...
    if not fc_ok:
        ...
        print(f"  - Qwen3-4B가 function calling을 지원하지 않을 수 있습니다.")
        print(f"  - OpenPipe로 데이터 수집 후 Unsloth로 fine-tuning이 필요합니다.")
        print(f"  - 또는 Hermes-3, Llama-3.1 등 function calling 지원 모델로 교체하세요.")
        print()
    ...
    if conn_ok and fc_ok:
        ...
        print(f"\n다음 단계:")
        print(f"  1. OpenPipe로 도구 호출 데이터 수집")
        print(f"  2. Unsloth로 Qwen3-4B fine-tuning")
        print(f"  3. Fine-tuned 모델을 llama.cpp로 로드")
    else:
